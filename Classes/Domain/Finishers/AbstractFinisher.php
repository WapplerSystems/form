...
abstract class AbstractFinisher implements FinisherInterface, LoggerAwareInterface
{
    ...
    /**
     * WapplerSystems fork: nullable, for the same reason as $eventDispatcher
     * above. Upstream declares this non-nullable and reads it unconditionally,
     * which turns a finisher built outside the DI container into a fatal
     * "must not be accessed before initialization" the first time an option is
     * read as a display value — inside a finisher, where this fork has learned
     * that a swallowed error costs the mail without leaving a trace. Unset
     * means no resolver, and the submitted value is passed through.
     */
    private ?FormValueResolver $formValueResolver = null;
    ...
    private function resolveReference(string $property, FormRuntime $formRuntime, bool $resolveDisplayValues)
    {
        ...
        if ($this->formValueResolver === null) {
            return $value;
        }
