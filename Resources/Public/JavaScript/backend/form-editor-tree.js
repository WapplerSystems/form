...
import { FORM_EDITOR_TREE_EVENTS } from './form-editor-tree-events.js';
 ...
 * validation rules.
 */
let FormEditorTree = class FormEditorTree extends Tree {
    /**
    ...
    constructor() {
        ...
        this.childValidationErrors = new Set();
    ...
    setNodes(nodes) {
        ...
        this.nodes = this.enhanceNodes(basicNodes);
     ...
     */
    async moveNode(node, target, position) {
     ...
     */
    isDropAllowedForFormEditor(draggingNode, targetNode, position) {
     ...
     */
    getTargetDepthAfterDrop(targetNode, position) {
        if (position === TreeNodePositionEnum.INSIDE) {
     ...
     */
    dispatchFormEditorEvent(eventName, detail) {
     ...
     */
    convertToTreeNodes(nodes, parentIdentifier = '', depth = 0) {
        const result = [];
        nodes.forEach((node) => {
            ...
            if (node.children && node.children.length > 0) {
                const childNodes = this.convertToTreeNodes(node.children, node.identifierPath, depth + 1);
